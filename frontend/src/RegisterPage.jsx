...
import { User, Lock, Eye, EyeOff, ArrowRight, Mail, AlertTriangle, Check, X, ShieldCheck, Sparkles } from 'lucide-react';
...
function RegisterPage() {
  ...
  const [emailSuggestion, setEmailSuggestion] = useState(''); // Lưu trữ email gợi ý sửa lỗi
  const navigate = useNavigate();

  // 🎯 DANH SÁCH CÁC NHÀ CUNG CẤP EMAIL LỚN ĐỂ SO SÁNH GỢI Ý
  const POPULAR_DOMAINS = ['gmail.com', 'yahoo.com', 'outlook.com', 'hotmail.com', 'icloud.com', 'fpt.edu.vn'];

  // 🎯 THUẬT TOÁN ĐO KHOẢNG CÁCH CHUỖI (LEVENSHTEIN DISTANCE) KHÔNG CẦN CÀI THƯ VIỆN
  const checkEmailTypo = (inputEmail) => {
    ...
    // Tìm xem domain người dùng gõ có sém giống với domain lớn nào không
    for (let targetDomain of POPULAR_DOMAINS) {
      if (domain === targetDomain) return ''; // Nếu gõ đúng hoàn toàn thì không hiện gợi ý

      // Tính khoảng cách sai lệch ký tự
      const distance = getLevenshteinDistance(domain.toLowerCase(), targetDomain);
      
      // Nếu chỉ sai lệch từ 1 đến 2 ký tự (Ví dụ: gml, gamil, yaho, outlok...)
      if (distance <= 2) {
  ...
  // Hàm phụ trợ tính toán độ tương đồng giữa 2 chuỗi văn bản
  ...
  const handleInputChange = (e) => {
    ...
    setFormData(prev => {
      ...
      // Nếu đang thay đổi ô email, chạy hàm kiểm tra typo để gợi ý real-time
      if (name === 'email') {
  ...
  // Hàm xử lý khi user click vào dòng chữ gợi ý màu vàng
  const applySuggestion = () => {
    if (emailSuggestion) {
      setFormData(prev => ({ ...prev, email: emailSuggestion }));
      setEmailSuggestion(''); // Áp dụng xong thì xóa dòng gợi ý đi
    }
  };
  ...
  const handleRegister = async (e) => {
    ...
    // 1. Kiểm tra định dạng Email hợp lệ cơ bản
    ...
    // 2. Kiểm tra điều kiện mật khẩu
    if (!isPasswordValid) {
      showToast("Please meet all password requirements first!", "error");
      return;
    }

    // 3. Kiểm tra khớp mật khẩu giữa 2 ô
    ...
    try {
      ...
      */
      ...
      // --- ĐOẠN GIẢ LẬP ĐỂ TEST ĐUÔI MAIL TRÙNG CŨ (XÓA KHI KẾT NỐI API THẬT) ---
      await new Promise((resolve, reject) => {
        setTimeout(() => {
          if (formData.email === 'admin@yahoo.com' || formData.email === 'staff@fpt.edu.vn') {
            reject(new Error('This email already exists in the database!'));
          } else {
            resolve();
          }
        }, 1500);
      ...
      showToast(err.message || "Registration failed due to network error.", "error");
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <form onSubmit={handleRegister}>
            <div className="input-group">
              ...
              <div className="input-wrapper">
                ...
                <input type="text" name="fullName" className="form-input" placeholder="John Doe" value={formData.fullName} onChange={handleInputChange} disabled={isLoading} required />
              </div>
            </div>

            <div className="input-group" style={{ marginBottom: emailSuggestion ? '0.25rem' : '1rem' }}>
              <label className="input-label">BUSINESS EMAIL</label>
              <div className="input-wrapper">
                <Mail size={16} className="input-icon-left" />
                <input type="email" name="email" className="form-input" placeholder="staff@company.vn" value={formData.email} onChange={handleInputChange} disabled={isLoading} required />
              </div>
            </div>

            {/* 🎯 GIAO DIỆN HIỂN THỊ GỢI Ý ĐỔI EMAIL KHI GÕ NHẦM */}
            {emailSuggestion && (
              <div style={{
                display: 'flex', alignItems: 'center', gap: '0.35rem',
                backgroundColor: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.2)',
                padding: '0.4rem 0.75rem', borderRadius: '0.375rem', marginBottom: '1rem',
                ...
                <span>
                  ...
                  <strong 
                    ...
                    style={{ color: '#3b82f6', textDecoration: 'underline', cursor: 'pointer', fontWeight: '700' }}
                  >
                    {emailSuggestion}
                  </strong>?
                </span>
            ...
            <div className="input-group" style={{ marginBottom: '1.5rem' }}>
              ...
              <div className="input-wrapper">
                <Lock size={16} className="input-icon-left" />
                <input type="password" name="confirmPassword" className="form-input" placeholder="••••••••" value={formData.confirmPassword} onChange={handleInputChange} disabled={isLoading} required />
              </div>
            </div>
            ...
            <button 
              ...
              style={{
                ...
                display: 'flex', alignItems: 'center', justifyYContent: 'center', gap: '0.5rem', 
                cursor: isLoading ? 'not-allowed' : 'pointer',
                transition: 'all 0.2s', marginBottom: '1.5rem', justifyContent: 'center'
              }}
            ...
            >
              {isLoading ? "Processing Request..." : "Authorize & Create Account"} 
